...
class Democracy(commands.Cog):
    def __init__(self, bot): #idk how to make this an external module properly (commands.Cog.listener() or other bs just makes the bot double the commands)
        self.bot = bot
        @bot.event
    ...
    async def add_law(self, ctx, keyword: str, law_name: str, description: str, enforce: bool = True):
        if law_name in laws_dict:
            ...
            return
        choice = await vote(self.bot, ctx, f"Добавить ли закон {law_name}?\nОписание закона:\n{description}", ["Да", "Нет"], symbols='thumbs', importance=Importance.medium)
        if choice.pop()==1:
