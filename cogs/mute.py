...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, member: discord.Member):
        choice = await vote(self.bot, ctx, f"Замутить ли пользователя {member.name}", ["Да", "Нет"], importance=Importance.medium)
        if choice==1:
    ...
    async def unmute(self, ctx, member: discord.Member):
        choice = await vote(self.bot, ctx, f"Размутить ли пользователя {member.name}", ["Да", "Нет"], importance=Importance.medium)
        if choice==1:
            await ctx.send("Голосование провалилось")
            return
